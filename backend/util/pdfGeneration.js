...
var getAnnexure3 = async (data) =>{
    ...
    doc.text("This is to inform you that your request for dealing in "+data.request_quantity+"(nos) shares of the Company as mentioned in your application dated "+request_date_string+" is "+data.request_status+".",{
        align: 'left'
        })
    if (data.request_status == "Approved"){
        doc.text("Please note that the said transaction must be completed on or before "+date_requested_to_string+" (date) that is within 7 days from today.")

        doc.moveDown()
        doc.moveDown()
        doc.text("In case you do not execute the approved transaction /deal on or before the aforesaid date you would have to seek fresh pre-clearance before executing any transaction/deal in the securities of the Company. Further, you are required to file the details of the executed transactions in the attached format within 2 days from the date of transaction/deal. In case the transaction is not undertaken a ‘Nil’ report shall be necessary.",{
            align: 'left'
        })    
    }
    else{
        doc.text("Request has been rejected for following reasons: \n "+data.reason,{
            align: 'left'
        })
        doc.moveDown()
        doc.moveDown()
...
var getAnnexure4 = async (data) =>{
    ...
    row_data.push([data.name,data.transaction_quantity,data.request_type,data.trans_folio,data.transaction_price])
    const doc = new PDFDocument({
        // layout : 'landscape'
    });
    // doc.pipe(fs.createWriteStream('example.pdf'));
    ...
    doc.text("I hereby inform that I have "+data.request_type+" "+data.transaction_quantity+" securities as mentioned below on "+transaction_date_string+"(date).",{
        align: 'left'
        })
    doc.moveDown()
    doc.moveDown()
    ...
    doc.moveDown()
    doc.moveDown()
    doc.text("In connection with the aforesaid transaction(s), I hereby undertake to preserve, for a period of 5 years and produce to the Compliance officer / SEBI any of the following documents:",{
        align: 'left'
        })
    doc.moveDown()
    doc.text("1. Broker’s contract note.\
    ...
    doc.moveDown()
    doc.text("I agree to hold the above securities for a minimum period of six months. In case there is any urgent need to sell these securities within the said period, I shall approach the Compliance Officer for necessary approval. (Applicable in case of purchase/ subscription).",{
        align: 'left'
        })
    doc.moveDown()
    doc.text("I declare that the above information is correct and that no provisions of the Company’s Code and/or applicable laws/regulations have been contravened for effecting the above said transactions(s).",{
        align: 'left'
        })
    doc.moveDown()
    doc.moveDown()
...
var getAnnexure6 = async (data) =>{
    ...
    doc.text("Details of Securities held on appointment of KMP or Director or upon becoming a Promoter or member of the promoter group of a listed company  and immediate relatives of such persons and by other such persons as mentioned in regulation 6(2).")
    ...
    doc.text("Category of person (KMP/Dir. or Promoter or member of the promoter group/Immediate relative to/others, etc)", x, y, { 
        width: 100,
        align: 'center'
      }
    );
    ...
    doc.text("Date of appointment of KMP/Dir or Date of becoming Promoter/member of the promoter group", x, y, { 
        width: 100,
        align: 'center'
      }
    );
    ...
    doc.text('Securities held at the time of appointment of KMP/Dir or upon becoming Promoter or member of the promoter group', x, y, { 
        width: 200,
        align: 'center'
      }
    );
    ...
    doc.text('Type of securities (For e.g.-Share, warrants, Convertible debentures, Rights entitlements, etc.)', x, y, { 
        width: 100,
        align: 'center'
      }
    );
    ...
    doc.text("Note: “Securities” shall have the meaning as defined under regulation 2(1) (i) of SEBI  (Prohibition of Insider Trading) Regulations, 2015", 50, 470)
    ...
    doc.text("Note:: For existing designated persons, this declaration to be given on an annual basis (as of 31 March) and for new designated persons within 30 days from becoming a Designated Person", 50)
...
var getAnnexure5 = async (data) =>{
    ...
    doc.text("Dear Sir,  ")
    doc.text("I, "+data.name+", hereby furnish the following details in terms of the "+company+" Insider Trading Policy:")
    var header_data = []
    var row_data = [] 
    ...
    row_data.push(["Degree and Names of the educational institutions where studied", data.last_institute])
    ...
    doc.text("3. Details of persons with whom I have a ‘material financial relationship’")
    ...
    doc.text("* “material financial relationship” shall mean a relationship in which"
    ...
    doc.text("** “immediate relative” means (i)  a spouse of a person and (ii) child,"
...
var getAnnexure7And8 = async (data) => {
    ...
    doc.text("Details of change in holding of Securities of Promoter, Member of the of the Promoter "
    ...
    doc.text("Category of person (Promoter/Member of the  promoter"
    +"group/designated person/Directors/Immediate "
    +" relative to/others etc.)", x, y, { 
        width: 100,
        align: 'center'
      }
    );
    ...
    doc.text('Date of allotment advice/acquisition shares/disposal of shares specify', x, y, { 
        width: 100,
        align: 'center'
      }
    );
    ...
    doc.text('Type of securities (For e.g.-Share, warrants, Convertible debentures, Rights  entitlements, etc. etc.', x, y, { 
        width: 50,
        align: 'center'
      }
    );
    ...
    doc.text('Transaction type (Purchase/Sale/pledge/revocation/Invocation others-please specify', x, y, { 
        width: 40,
        align: 'center'
      }
    );
    ...
    x  = x+100
    doc.text('Mode of acquisition/disposal(on market/public/rights/preferential offer/off market/Inter-se transfer, ESOPs, etc.)', x, y, { 
        width: 200,
        align: 'center'
      }
    );
    ...
    doc.text("Note: “Securities” shall have the meaning as defined under regulation 2(1) (i) of SEBI  (Prohibition of Insider Trading) Regulations, 2015", 50, 320)
...
var getAnnexure1And2=async (req, EmployeeData, FolioData, db,requestId)=>{
    ...
    row_data.push(["Proposed date of dealing in securities",proposed_dealing_from_date_str])
    row_data.push(["Estimated number of securities proposed to be acquired/subscribed/sold",proposed_quantity])
    row_data.push(["Price at which the transaction is proposed",proposed_price])
    row_data.push(["Current market price (as on date of application)",market_price])
    row_data.push(["Whether the proposed transaction will be through stock exchange or off-market deal",mode])
    row_data.push(["Folio No. / DP ID / Client ID No. where the securities will be credited / debited",transaction_folio])
    const doc = new PDFDocument({
        // layout : 'landscape'
    });
    // doc.pipe(fs.createWriteStream('example.pdf'));
    ...
    doc.moveDown()
    doc.text("Application for Pre-dealing approval in securities of the Company",{
        align: 'left'
        })
    doc.moveDown()
    doc.text("Pursuant to the SEBI (prohibition of Insider Trading) Regulations, 2015 as amended and the Company’s Insider Trading Policy, I seek approval to purchase / sale of "+proposed_quantity+" equity shares of the Company as per details given below:",{
        align: 'left'
        })
    doc.moveDown()
    doc.table(table0, {
    ...
    doc.moveDown()
    doc.text("FORMAT OF UNDERTAKING TO BE ACCOMPANIED WITH \nTHE APPLICATION FOR PRE-CLEARANCE",{
        align: 'center'
        })
    doc.moveDown()
    doc.moveDown()
    ...
    doc.moveDown()
    doc.moveDown()
    doc.text("I, "+KMP_Name+" "+designation+", of the Company residing at "+company_add+", am desirous of dealing in "+proposed_quantity+" shares of the Company as mentioned in my application dated "+dateStr+" for pre-clearance of the transaction.",{
            align: 'left'
            })
    doc.moveDown()
    doc.text("I further declare that I am not in possession of or otherwise privy to any unpublished Price Sensitive Information (as defined in the Company’s Code of Conduct for prevention of Insider Trading (the Code) up to the time of signing this Undertaking.",{
            align: 'left'
            })
    doc.moveDown()
    doc.text("In the event that I have access to or received any information that could be construed as “Price Sensitive Information” as defined in the Code, after the signing of this undertaking but before executing the transaction for which approval is sought, I shall inform the Compliance Officer of the same and shall completely refrain from dealing in the securities of the Company until such information becomes public.I declare that I have not contravened the provisions of the Code as notified by the Company from time to time.",{
            align: 'left'
            })
    doc.moveDown()
    doc.text("I undertake to submit the necessary report within four days of execution of the transaction / a ‘Nil’ report if the transaction is not undertaken.\nIf approval is granted, I shall execute the deal within 7 days of the receipt of approval failing which I shall seek pre-clearance.",{
            align: 'left'
            })
    doc.moveDown()
    doc.text("I declare that I have made full and true disclosure in the matter.",{
            align: 'left'
            })
    doc.moveDown()
    doc.moveDown()
...
const getViloationsPdf = async (body) => {
    ...
    try {
        ...
        var headers = ["Emp. Code", "PAN", "Name", "Buy/Sell", "BENPOS Date", "Curr. Total Share", "Prev. Total Share", "Req. Status", "Appr. Date", "Folio-1", "Folio-2", "Folio-3", "Folio-4", "Folio-5"];
        var tbody = [];
        for(var i=0; i<body.length; i++) {
            var row = [];
            row[0] = body[i].code;
...
const getRequestPdf = async (body) => {
    try {
        ...
        var headers = ["Id", "Name", "Category", "Security Type", "Mode", "Tr. Folio", "Request Type", "Valid Until", "Proposed Quantity", "Proposed Price", "Timestamp"];
        var tbody = [];
        for (var i=0; i<body.length; i++) {
            var row = [];
            row[0] = body[i].id;
