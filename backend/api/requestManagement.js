...
module.exports = (app, db) =>
{
            ...
            if(nowDate.getTime() > date_requested_to.getTime()){
                ...
                if(window_close_from.getTime() <= base_date.getTime() && base_date.getTime() <= window_close_to.getTime()){
                    ...
                    if(empData.length == 0){
                        ...
                        var updatedRel = await db.Relatives.update({"security_type": security_type},{
                            where:{
                                pan: pan,
                                is_active: true
                            }
                        ...
                        var updatedEmp = await db.Employees.update({"security_type": security_type},{
                            where:{
                                pan: pan,
                                is_active: true
                            }
                    ...
                    // Pdf creation part
                    data = {reqId: req.params.id,name: name,transaction_quantity: transaction_quantity,
                        request_type: request_type,trans_folio: trans_folio,transaction_date: transaction_date,
                        transaction_price: transaction_price,company: company}
                    ...
                    res.setHeader('Content-disposition', 'attachment; filename=requestComplete-'+req.params.id+'.pdf');
    ...
    app.post('/request/:id/complete', async (req,res) => {
        ...
        .then(async data => {
            var activityData = {"activity": "Annexure-7 & Annexure-8 generated","description": "",
                                "done_by": [req.user.userId],
            ...
            activityData = {"activityId": activity_id,
                                "done_for": [data.Folio.Employee.id]}
            activity_id = await trackActivity(activityData, db)
            res.setHeader('Content-disposition', 'attachment; filename=Annexure7-8.pdf');
            res.set({"id": req.params.id})
            res.header("Access-Control-Expose-Headers", "id");
            annex7.pipe(res)
        })
    ...
    app.post("/request", async (req,res) => {
        try{
            ...
            activity_id = await trackActivity(activityData, db)
            res.setHeader('Content-disposition', 'attachment; filename=newRequest-'+requestId+'.pdf');
            res.setHeader('Content-type', 'application/pdf');
    ...
    app.put("/request", async (req,res) => {
        try{
            ...
            if(request_status == "Approved"){
                var activityData = {"activity": "Pre-transaction Request Approved","description": "",
                                "done_by": [req.user.userId],
                                "done_for": [requestData.Folio.Employee.id]}
                activity_id = await trackActivity(activityData, db)
                newRequestData = {"date_requested_from": date_requested_from,"date_requested_to": date_requested_to,
                                   "request_status": request_status,"approval_date": new Date()}
            }
            else{
                var activityData = {"activity": "Pre-transaction Request Rejected","description": "",
                                "done_by": [req.user.userId],
                                "done_for": [requestData.Folio.Employee.id]}
                activity_id = await trackActivity(activityData, db)
                newRequestData = {"request_status": request_status,"approval_date": new Date(),"reason": req.query.reason}
            }
            ...
            // pdf creation
            data = {KMP_Name: KMP_Name,designation: designation,
                    add: add,requestData_id: requestData.id,company: company,
                    request_status: request_status,date_requested_to: date_requested_to,
                    request_quantity: request_quantity,request_date: request_date,reason: req.query.reason}
            ...
            var activityData = {"activityId": activity_id}
            activity_id = await trackActivity(activityData, db)
            res.setHeader('Content-disposition', 'attachment; filename=requestStatust-'+req.query.id+'.pdf');
            res.setHeader('Content-type', 'application/pdf');
