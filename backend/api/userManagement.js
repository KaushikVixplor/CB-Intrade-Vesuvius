...
var refreshAccessTokenSecret = require('../config/config')[env]['refreshAccessTokenSecret']
...
module.exports = (app, db) =>
{
    ...
    app.post('/login',async (req, res) => {
        ...
        .then(userDetails =>{
            if(!userDetails) {
                ...
                res.status(401).json({'message': 'Employee not active...Contact admin'})
            }
            else if(userDetails && bcrypt.compareSync(req.body.password, userDetails.password)){
                user_pan = userDetails.pan;
                user_company = userDetails.company_id;
                user_id = userDetails.id;
                const accessToken = jwt.sign({userId: user_id, companyId: user_company, userPAN: user_pan, is_compliance: userDetails.is_compliance}, accessTokenSecret, {expiresIn:expTime});
                const refreshAccessToken = jwt.sign({userId: user_id, companyId: user_company, userPAN: user_pan, is_compliance: userDetails.is_compliance}, refreshAccessTokenSecret);
                // console.log("ID", user.email, refreshAccessToken);
                db.Employees.update({refreshAccessToken:refreshAccessToken},{
                    where:{
                        id: user_id
                    }
                })
    ...
    app.post("/logout", (req,res) => {
        db.Employees.update({refreshAccessToken:""},{
            where:{
                id:req.user.userId
            }
        })
     ...
     app.put("/user/:userId/resetpassword", async (req,res) => {
            var activityData = {"activity": "Reset Password","description": "",
            ...
            .catch(err => {
                ...
                res.status(500).json({message:"Database error in reset password:: "+err});
            })
        })
        ...
        try
        {
            ...
            var out  =  await localUpload.fields([{ name: 'employeeData', maxCount: 1}])(req, res, async function(err) {
                ...
                try
                {
                            ...
                            for(i=0;i<empData.length;i++){
                                const result = await db.sequelize.transaction(async (t) => {
                                    try{
                                        ...
                                        var last_institute = record.LastDegreeANDEducationalInstitution|| ''
                                        var last_employer = record.NameOfPastEmployer|| ''
                                        var other_identifier_type = record.OtherIdentifierType|| ''
                                        ...
                                        if(record.Status.toLowerCase() == 'self'){
                                            ...
                                            if(DateOfAppointmentAsInsider != ''){
                                                ...
                                                if(DateOfAppointmentAsInsider.toString().includes("/")){
                                                    ...
                                                    appointment_date = await ExcelDateToJSDate(DateOfAppointmentAsInsider)
                                                }
                                            } 
                                            else{
                                            ...
                                            if(LastBenposDate != ''){
                                                if(LastBenposDate.toString().includes("/")){
                                                    ...
                                                    last_benpos_date = await ExcelDateToJSDate(LastBenposDate)
                                                }
                                            } 
                                            else{
                                            ...
                                            if("FolioNo1" in record){
                                                if("FShares1" in record){
                                                    Folio_info.push({"emp_pan": pan,"folio": record.FolioNo1,"current_share": record.FShares1})
                                                    total_share = total_share + record.FShares1
                                                }
                                                else{
                                                    Folio_info.push({"emp_pan": pan,"folio": record.FolioNo1,"current_share": 0})
                                                    total_share = total_share + 0
                                                }
                                            }
                                            if("FolioNo2" in record){
                                                if("FShares2" in record){
                                                    Folio_info.push({"emp_pan": pan,"folio": record.FolioNo2,"current_share": record.FShares2})
                                                    total_share = total_share + record.FShares2
                                                }
                                                else{
                                                    Folio_info.push({"emp_pan": pan,"folio": record.FolioNo2,"current_share": 0})
                                                    total_share = total_share + 0
                                                }
                                            }
                                            if("FolioNo3" in record){
                                                if("FShares3" in record){
                                                    Folio_info.push({"emp_pan": pan,"folio": record.FolioNo3,"current_share": record.FShares3})
                                                    total_share = total_share + record.FShares3
                                                }
                                                else{
                                                    Folio_info.push({"emp_pan": pan,"folio": record.FolioNo3,"current_share": 0})
                                                    total_share = total_share + 0
                                                }
                                            }
                                            if("FolioNo4" in record){
                                                if("FShares4" in record){
                                                    Folio_info.push({"emp_pan": pan,"folio": record.FolioNo4,"current_share": record.FShares4})
                                                    total_share = total_share + record.FShares4
                                                }
                                                else{
                                                    Folio_info.push({"emp_pan": pan,"folio": record.FolioNo4,"current_share": 0})
                                                    total_share = total_share + 0
                                                }
                                            }
                                            if("FolioNo5" in record){
                                                if("FShares5" in record){
                                                    Folio_info.push({"emp_pan": pan,"folio": record.FolioNo5,"current_share": record.FShares5})
                                                    total_share = total_share + record.FShares5
                                                }
                                                else{
                                                    Folio_info.push({"emp_pan": pan,"folio": record.FolioNo5,"current_share": 0})
                                                    total_share = total_share + 0
                                                }
                                            }
                                            ...
                                            if("FolioNo1" in record){
                                                if("FShares1" in record){
                                                    Folio_info.push({"emp_pan": emp_pan,"emp_relative_pan": pan,"folio": record.FolioNo1,"current_share": record.FShares1})
                                                    total_share = total_share + record.FShares1
                                                }
                                                else{
                                                    Folio_info.push({"emp_pan": emp_pan,"emp_relative_pan": pan,"folio": record.FolioNo1,"current_share": 0})
                                                    total_share = total_share + 0
                                                }
                                            }
                                            if("FolioNo2" in record){
                                                if("FShares2" in record){
                                                    Folio_info.push({"emp_pan": emp_pan,"emp_relative_pan": pan,"folio": record.FolioNo2,"current_share": record.FShares2})
                                                    total_share = total_share + record.FShares2
                                                }
                                                else{
                                                    Folio_info.push({"emp_pan": emp_pan,"emp_relative_pan": pan,"folio": record.FolioNo2,"current_share": 0})
                                                    total_share = total_share + 0
                                                }
                                            }
                                            if("FolioNo3" in record){
                                                if("FShares3" in record){
                                                    Folio_info.push({"emp_pan": emp_pan,"emp_relative_pan": pan,"folio": record.FolioNo3,"current_share": record.FShares3})
                                                    total_share = total_share + record.FShares3
                                                }
                                                else{
                                                    Folio_info.push({"emp_pan": emp_pan,"emp_relative_pan": pan,"folio": record.FolioNo3,"current_share": 0})
                                                    total_share = total_share + 0
                                                }
                                            }
                                            if("FolioNo4" in record){
                                                if("FShares4" in record){
                                                    Folio_info.push({"emp_pan": emp_pan,"emp_relative_pan": pan,"folio": record.FolioNo4,"current_share": record.FShares4})
                                                    total_share = total_share + record.FShares4
                                                }
                                                else{
                                                    Folio_info.push({"emp_pan": emp_pan,"emp_relative_pan": pan,"folio": record.FolioNo4,"current_share": 0})
                                                    total_share = total_share + 0
                                                }
                                            }
                                            if("FolioNo5" in record){
                                                if("FShares5" in record){
                                                    Folio_info.push({"emp_pan": emp_pan,"emp_relative_pan": pan,"folio": record.FolioNo5,"current_share": record.FShares5})
                                                    total_share = total_share + record.FShares5
                                                }
                                                else{
                                                    Folio_info.push({"emp_pan": emp_pan,"emp_relative_pan": pan,"folio": record.FolioNo5,"current_share": 0})
                                                    total_share = total_share + 0
                                                }
                                            }
                                            ...
                                            var newRelativeInfo = await db.Relatives.create(RelativeData,{transaction: t})      
                                        ...
                                        errorList.push({name: name, message: "error while employee add:: "+error})
                                        // throw error
                                    }
                                })
                            }
                    ...
                    res.status(500).json({'message': "cann't upload employeeData:: "+error})
                }    
            })
        }
        catch(error){
    ...
    app.post('/user', async (req,res) => {
        // Activity add 
        var activityData = {"activity": "Connected Person Add","description": "",
                            "done_by": [req.user.userId],
                            "done_for": []}
        var activity_id = await trackActivity(activityData, db)
        ...
        if(req.user&&req.user.is_compliance){
            ...
            if(empData.length > 0){
                res.status(400).json({message:"This email Id already exist in dataBase try with another email Id", empData}) 
            }
            else{
                ...
                var variables = [CompanyData.name,CompanyData.website,backendUrl,req.body.email,tempPass,CompanyData.name,coData.name]
    ...
    app.get('/company', (req,res) => {
        // company fetch 
        db.Company.findAll({
            attributes: ["id","code","isin","cin","name","email","address",'window_close_from','window_close_to','purpose']      
        })
        .then(data => {
        ...
        .then(data => {
            ...
            .then(async (admin) => {
                ...
                if(data.status == 'Temp'){
                    ...
                    var activityData = {"activity": "Personal Details Correction Request","description": "",
                                        "done_by": [req.params.id],
                                        "done_for": [req.params.id]}
                    activity_id = await trackActivity(activityData, db)
                    ...
                    if(req.body.status == "Approved"){
                        var activityData = {"activity": "Personal Details Correction Request Approved","description": "",
                                            "done_by": [req.user.userId],
                                            "done_for": [req.params.id]}
                        activity_id = await trackActivity(activityData, db)
                        updateData = {
                        ...
                        var activityData = {"activity": "Personal Details Correction Request Rejected","description": "",
                                            "done_by": [req.user.userId],
                                            "done_for": [req.params.id]}
                        activity_id = await trackActivity(activityData, db)
                        updateData = {
                ...
                if(mailOb.flag){
                    ...
                    var newText = mailOb.type == 'FIRST_ACTIVE'?
                    'New_cp_added':
                    mailOb.type == 'REQUEST_UPDATE'?
                    'Cp_update_request':
                    mailOb.type == 'RE_ACTIVE'?
                    ...
                    var mailIds = mailOb.type == 'FIRST_ACTIVE'?
                    admin.email:
                    mailOb.type == 'REQUEST_UPDATE'?
                    admin.email:
                                        ...
                                        try{
                                            ...
                                            if(finFol){
                                                ...
                                                if(fol.id){
                                                    ...
                                                    try{
                                                        var newRel = await db.Folios.create({...fol,emp_pan: req.body.pan, emp_relative_pan:rel.pan})
    ...
    app.put('/user/:id/release', async (req,res) => {
        try{
            var activityData = {"activity": "Release Connected Person","description": "",
                                "done_by": [req.user.userId],
                                "done_for": [req.params.id]}
            var activity_id = await trackActivity(activityData, db)
            var nowDate = new Date()
            var newEmployyeData = await db.Employees.update({status: 'Deactive',is_active: false,release_date: nowDate},{
                where:{
                    id: req.params.id
                }
            })
            if(newEmployyeData[0] > 0){
                ...
                var newRelativeData = await db.Relatives.update({status: 'Deactive',is_active: false,release_date: nowDate},{
                    where:{
                        emp_pan: employyeData.pan
                    }
                })
                var newFolioData = await db.Folios.update({status: 'Deactive',is_active: false,release_date: nowDate},{
                    where:{
                        emp_pan: employyeData.pan
                    }
                })
                ...
                res.status(200).json({message:"No Employee with ID: "+req.params.id})
            }
        }
        catch(error){
    ...
    app.put('/relative/:id/release', async (req,res) => {
        try{
            var activityData = {"activity": "Release Relative","description": "",
                                "done_by": [req.user.userId],'done_for_relative':true,
                                "done_for": [req.params.id]}
            var activity_id = await trackActivity(activityData, db)
            var nowDate = new Date()
            var newRelativeData = await db.Relatives.update({status: 'Deactive',is_active: false,release_date: nowDate},{
            ...
            if(newRelativeData[0] > 0){
                ...
                res.status(200).json({message:"No Relative with ID: "+req.params.id})
            }
        }
        catch(error){
    ...
    app.put('/users/canEdit', async (req,res) => {
        try{
            ...
            for(var i=0 ; i < data.length ; i++){
                try{
                    var newEmpData = await db.Employees.update({canEdit: data[i].canEdit},{
                        where:{
                            id: data[i].id
                        }
                    })
            ...
            res.status(200).json({message:"Employee can Edit update successfullly"})
        }
        catch(error){
            console.error("Employee can Edit update error", error);
            res.status(500).json({message:"Employee can Edit update error:: "+error})
        }
    })
