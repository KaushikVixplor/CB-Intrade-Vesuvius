const getConnectedPersons = require('../util/pdfGeneration').getConnectedPersons;
const getConnectedPersonsExcel = require('../util/excelGeneration').getConnectedPersonsExcel;
...
module.exports = (app, db) => {
	...
	//Get pdf of compareTransaction
	app.get('/compareTransactionPdf', async (req, res) => {
		try {
			...
			compareTransactionNew(transactionData)
				.then((data) => {
					...
					getCompareTransactionPdf(data, transactionData.prev_benpos_date, transactionData.current_benpos_date)
						...
						.catch((err) => {
							console.log(err);
							res.status(500).json({ message: "error while compare transaction download:: "+err });
						});
				})
				.catch((err) => {
					console.log(err);
	...
	//Get list of violated transactions list as pdf
	app.get('/violationPdf', async (req, res) => {
		try {
			fromDateStr = req.query.startDate;
			toDateStr = req.query.endDate;
			...
			getViolationData(transactionData)
				.then((data) => {
					console.log('Get violation data successfully');
					getViloationsPdf(data)
						.then((doc) => {
							console.log('Violation data converted to pdf successfully');
							res.setHeader('Content-disposition', 'attachment; filename=Violation.pdf');
							res.setHeader('Content-type', 'application/pdf');
							doc.pipe(res);
						})
						.catch((err) => {
							console.log(err);
							res.status(500).json({ message: "error while Violation download:: "+err });
						});
				})
				.catch((err) => {
					console.log(err);
	...
	//Get activity log as pdf
	app.get('/activityPdf', async (req, res) => {
		try {
			fromDateStr = req.query.startDate;
			toDateStr = req.query.endDate;
			console.error('fromDateStr = ', fromDateStr);
			...
			getActivityPdf(ActivityLogsData)
				.then((doc) => {
					console.log('Activity log data converted to pdf successfully');
				...
				.catch((err) => {
					console.log(err);
					res.status(500).json({ message: "error while activity log download:: "+err });
				});
		} catch (err) {
			console.log(err);
			res.status(500).json({ message: "error while activity log download:: "+err });
	...
	//Download Requested Transaction as pdf
	app.get('/requestPdf', (req, res) => {
		db.Requests
			.findAll({
				include: [
					{
			...
			.then((data) => {
				...
				getRequestPdf(data)
					.then((doc) => {
						console.log('Request data converted to pdf successfully');
					...
					.catch((err) => {
						console.log(err);
						res.status(500).json({ message: "error while request download:: "+err });
					});
			})
			.catch((err) => {
