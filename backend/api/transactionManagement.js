...
module.exports = (app, db) =>{
    ...
    app.get('/transaction',async (req, res) =>{
        try{
            ...
            if(prev_benpos_date.getTime() == current_benpos_date.getTime()){
                transactionData = {prev_benpos_date: prev_benpos_date,prev_benpos_data: [],
   ...
   async function subMonths(baseDate,months){
            ...
            if(window_close_from != null || window_close_to != null){
                ...
                var reqData1 = await db.Requests.findAll({
                    where:{
                        ...
                        transaction_date: {[Op.between] : [window_close_from,window_close_to]}
                    }
                })
                ...
                if((window_close_from.getTime() < shrChangeDate.getTime()) && (shrChangeDate.getTime() < window_close_to.getTime())){
            ...
            if(reqData.length > 0){
                for(var r=0;r<reqData.length;r++){
                    ...
                    if(reqData[r].request_quantity >= share){
                        ...
                        if((date_requested_from.getTime() < currDate.getTime()) && (currDate.getTime() < date_requested_to.getTime())){
                            ...
                            if(transaction_date != null){
                                ...
                                if((date_requested_from.getTime() < transaction_date.getTime()) && (transaction_date.getTime() < date_requested_to.getTime())){
    ...
    async function isFolioExists(pan,folio,current_share){
        try{
            ...
            if(updatedFolioInfo.length > 0){
                ...
                var newData = {"id": updatedFolioInfo[0].id,"emp_pan": updatedFolioInfo[0].emp_pan,"emp_relative_pan": updatedFolioInfo[0].emp_relative_pan,"folio": updatedFolioInfo[0].folio,"current_share": updatedFolioInfo[0].current_share}
                return [true,newData]
            }
            else{
                ...
                if(EmpInfo.length > 0){
                    ...
                    var newData = {"emp_pan": pan,"folio": folio,"current_share": current_share}
                    ...
                    if(EmpRelativeInfo.length > 0){
                        var newData = {"emp_pan": EmpRelativeInfo[0].emp_pan,"emp_relative_pan": pan,"folio": folio,"current_share": current_share}
                        return [false,newData]
                    }
                    else{
    ...
    app.post('/weeklyData',async (req, res) =>{
        ...
        try
        {
            var activityData = {"activity": "Benpos Data Upload","description": "",
            ...
            var out  =  await localUpload.fields([{ name: 'weeklyData', maxCount: 1}])(req, res, async function(err) {
                ...
                try
                {
                    ...
                    if (req.files)  {
                        ...
                        {
                            var errorList = []
                            req.body["weeklyData"] = localgetPublicUrl( req.files['weeklyData'][0].filename?req.files['weeklyData'][0].filename:
                            ...
                            // for(i=0;i<weeklyData.length;i++){
                                try{
                                    ...
                                    if(fileDate.getTime() != inputDate.getTime()){
                                        ...
                                        res.status(400).json({'message': "data already exists for this benpose date "})
                                    }
                                    else{
                                        var variationDatas = []
                                        var updatedDataId = []
                                            ...
                                            try{
                                                for(var i=0;i<weeklyData.length;i++){
                                                    try{
                                                        ...
                                                        console.error("error while updating transaction info in loop:: ",error)
                                                        errorList.push({ user: record.NAME, message: "error while updating benpose of user(" + record.NAME + ") pan(" + record.FPAN_NO + "):: " + error })
                                                        throw error
                                                    }
                                                }
                                                return true
                                                ...
                                                console.error("error while updating transaction info:: ",error)
                                                throw error
                                            }
                                        });
                    ...
                    res.status(500).json({"errorList": errorList, 'message': "cann't upload weeklyData:: "+error})
                }    
            })
        }
        catch(error){
            console.error("Error in weeklyData addition")
            res.status(500).json({"errorList": errorList, 'message': "cann't add weeklyData:: "+error})
        }
        });
